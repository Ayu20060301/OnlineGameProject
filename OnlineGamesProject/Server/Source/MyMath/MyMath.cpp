...
// ヨー行列を取得
MATRIX MyMath::MatRotationYaw(float yaw)
{
	MATRIX result = MatCreate();
...
// ロール行列を取得
MATRIX MyMath::MatRotationRoll(float roll)
{
	MATRIX result = MatCreate();
...
float MyMath::MinF(float valueA, float valueB)
{
	if (valueA < valueB)
	{
		return valueA;
...
float MyMath::MaxF(float valueA, float valueB)
{
	if (valueA > valueB)
	{
		return valueA;
...
float MyMath::ClampF(float value, float min, float max)
{
	float result = value;
...
float MyMath::Absolute(float value)
{
